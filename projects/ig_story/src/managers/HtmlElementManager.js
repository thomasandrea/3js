...
export default class HtmlElementManager {
  ...
  addMultipleElements(elementsConfig, type = 'fade') {
    elementsConfig.forEach(config => {
      if (type === 'fade') {
        this.addFadeElement(config.id, config.startZ, config.endZ, config.options);
      } else if (type === 'text') {
        this.addTextElement(config.id, config.startZ, config.endZ, config.options);
      }
    });
    return this;
  }
